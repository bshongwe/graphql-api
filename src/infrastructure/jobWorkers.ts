  ...
  try {
    ...
    switch (type) {
      ...
      case JOB_TYPES.SEND_PASSWORD_RESET:
        logger.info({ jobId: job.id, to }, 'Password reset email sent successfully');
        break;
      default:
  ...
  try {
    ...
    switch (type) {
      case JOB_TYPES.PROCESS_USER_SIGNUP:
        ...
        logger.info({ jobId: job.id, userId }, 'User signup processing completed');
        break;
      ...
      case JOB_TYPES.PROCESS_USER_DELETION:
        ...
        logger.info({ jobId: job.id, userId }, 'User deletion processing completed');
        break;
...
async function processNotificationJob(job: Job<NotificationJobData>) {
  ...
  logger.info({ jobId: job.id, userId, channel, title }, 'Processing notification job');
  ...
  try {
    ...
    logger.info({ jobId: job.id, userId, channel }, 'Notification sent successfully');
...
async function processDataExportJob(job: Job<DataExportJobData>) {
  ...
  logger.info({ jobId: job.id, type, userId, format }, 'Processing data export job');
  ...
  try {
    ...
    switch (type) {
      case JOB_TYPES.EXPORT_USER_DATA:
        ...
        logger.info({ jobId: job.id, userId, format }, 'User data export completed');
        break;
...
export const workers = {
  ...
  notificationsWorker: new Worker(JOB_QUEUES.NOTIFICATIONS, processNotificationJob, { 
    connection: redis,
    concurrency: 8,
    limiter: {
      max: 50,
